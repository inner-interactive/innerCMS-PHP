				...
				<div>
					<strong> <a href="<?=$viewLink?>"
						title="<?=$dbData[$i]['subject']?> 내용 보기">
									<?php echo $del_in.$dbData[$i]['subject'].$del_out?>
								</a>
					</strong>
					<!--
							<p>
								<a href="<?=$viewLink?>" title="<?=$dbData[$i]['subject']?> 내용 보기">
									<?php echo strcut(htmlspecialchars_decode($dbData[$i]['memo']),150)?>
								</a>
							</p>
							<span class="wdate"><?php echo str_replace("-",".",substr($dbData[$i]['writetime'],0,10))?></span>
							-->
				</div>
			</li>
				<?php }?>
