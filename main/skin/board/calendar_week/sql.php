...
/*
    ...
    */
    ...
    if( isset($_GET['date']) != null && (stripos($_GET['date'],'-') == 4) && strlen($_GET['date']) == 7 )
    {
        
        //검색하고자 하는 날짜
        $search_date = explode("-",$_GET['date']);
        $year = intval($search_date[0]);
        $month = intval($search_date[1]);
        $day = 1;
    ...
    if(isset($_GET['weekNum']) == NULL)
    {
        //현재 주에 해당하는 weekNum 값 구하기
        $weekNum = ceil((date("d",time()) - date("w",time())) / 7);
    }
    else
    {
        $weekNum = intval($_GET['weekNum']);
    }
    
    
    if($weekNum <= 0 || $weekNum > getTotalWeek($ym)) $weekNum = 1;
    ...
    $st_date_stamp = strtotime($m_start_date) - 60 * 60 * 24 * $m_start_date_weeknum + ( 60 * 60 * 24 * 7 * ($weekNum) );
    $start_date = date("Y-m-d", $st_date_stamp);
    $end_date = date("Y-m-d", $st_date_stamp + ( 60 * 60 * 24 * 6 ));
    ...
    {
        if(intval($month) <= 1){
            $prev_m = ( intval($year) - 1 )."-"."12";
        }else{
            $prev_m = $year."-".digits(intval($month)-1);
        }
        $prev_week = getTotalWeek($prev_m);
    }
    else
    {
        $prev_m = $year."-".digits($month);
        $prev_week = $weekNum-1;
    }
    
    //다음주 버튼
    if($weekNum >= getTotalWeek($ym))
    {
        if(intval($month) >= 12){
            $next_m = ( intval($year) + 1 )."-"."01";
        }else{
            $next_m = $year."-".digits(intval($month)+1);
        }
        $next_week = 1;
    }
    else
    {
        $next_m = $year."-".digits($month);
        $next_week = $weekNum + 1;
    }
