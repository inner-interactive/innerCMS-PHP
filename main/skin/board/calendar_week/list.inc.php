...
<div class="page_list">
	...
	<?php }?>
            ...
            for ($i = 0; $i < count($dbData); $i ++) {
				...
				<tr>
					...
					<td class="left">
						...
						<a href="<?=getBackUrl("menu|pno|category|limit|sfv|".$_GET['sfv']."|opt")?>&amp;mode=update&amp;no=<?=$dbData[$i]['indexcode']?>"
		...
		<div class="btnbox">
			...
			<a class="btn btn-default" href="<?=getBackUrl("menu|pno|category|limit|sfv|".$_GET['sfv']."|opt")?>&amp;mode=write" title="일정등록">일정등록</a>
			<?php }?>
			<a class="btn btn-default" href="<?=getBackUrl("menu|pno|category|limit|sfv|".$_GET['sfv']."|opt")?>&amp;mode=calendar" title="달력보기">달력보기</a>
		</div>
