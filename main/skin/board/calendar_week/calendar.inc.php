<div class="calendar-box">
	...
	<div class="month">
		<ul>
			<li><a href="<?=getBackUrl("menu")?>&amp;date=<?=$prev_m?>&amp;weekNum=<?=$prev_week?>"
				class="date_btn prev"><img src="<?=SKIN_URL?>img/cal-prev.png"
					alt="이전달 버튼" /></a></li>
			<li><a href="<?=getBackUrl("menu")?>&amp;date=<?=$next_m?>&amp;weekNum=<?=$next_week?>"
	...
	<ul class="weekdays">
		<li style="width:20%;">요일</li>
		<li style="width:80%;">행사</li>
	</ul>
	<ul class="days">
	<?php
    ...
    for ($i = 0; $i < 7; $i ++) {
        ...
        if ($day_info[$index]["isToday"] == "yes")
            $isToday = true;
        ?>
			<li class="<?=$_class?>" style="width:20%;">
    			<div class="daynum"><?=substr($index,8,2)?><span class="mweekday">(<?=$WEEKDAY[$_w]?>)</span>
    			</div>
			</li>
			<li class="<?=$_class?>" style="width:80%; border-right:none;">
			<?php
            for ($k = 0; $k < count($day_info[$index]['data']); $k ++) {
                ...
                $_thumb = $_data['thumb'];
                if (count($_thumb) > 0) {
                    $src = "../data/upload/" . $_thumb[0]['attach_file_name'];
                } else
                    $src = "";
