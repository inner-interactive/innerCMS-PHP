	...
	<div class="commentView">
		...
		<?php for($i = 0; $i < count($cdb); $i++){ if(intval($cdb[$i]['delflag']) == 1){$del_in="<del>"; $del_out="</del>";}else{$del_in = $del_out = "";}?>
			<li>
				<dl class="cominf">
					<dt>
						<span <?php if($cdb[$i]['commentrank'] > 0){ echo " class=\"rank".$cdb[$i]['commentrank']."\""; }?>>
						<?php  if($cdb[$i]['commentrank'] > 0){?><img src="img/skin/re.gif" alt="답변글" /><?php }?><?=$cdb[$i]['uname']?>
						</span>
						...
						<fieldset>
							...
							<?php }else{	// 비로그인 사용자 일 경우에?>
								<p class="writerinformation">
    								<label for="unameinner<?=$i?>">이름</label>
    								<input type="text" name="uname" id="unameinner<?=$i?>" value="<?=$userName?>" /> 
    								<label for="upwinner<?=$i?>">비밀번호</label>
    								<input type="password" name="upw" id="upwinner<?=$i?>" value="" style="ime-mode: disabled;" />
    							</p>
							<?php }?>
		...
		<form action="comment.php" method="post" enctype="multipart/form-data">
			<fieldset>
				...
				<p class="writerinformation">
					...
					<input type="text" name="uname" id="uname" value="" required /> <label for="upw">비밀번호</label>
					<input type="password" name="upw" id="upw" value="" required />
				</p>
