...
$column = $DB->getColumns(MENU_TABLE, array('menu_id', 'route'));
...
$query = "SELECT rank FROM ".MENU_TABLE." WHERE site = '".trim($_POST['site'])."' AND menu_id = ".$parent_id;
$dbData = $DB->getDBData($query);
$_POST['rank'] = isset($dbData[0]['rank']) ? intval($dbData[0]['rank']) + 1 : 1;

$query = "SELECT max(menu_order) FROM ".MENU_TABLE." WHERE site = '".trim($_POST['site'])."' AND parent_id = ".$parent_id;
$dbData = $DB->getDBData($query);
$_POST['menu_order'] = intval($dbData[0][0]) + 1;


$query = $DB->insertSql($column, $_POST, MENU_TABLE);
$DB->runQuery($query);

$query = "SELECT max(menu_id) FROM ".MENU_TABLE;
$dbData = $DB->getDBData($query);
...
if($menu_type == 'contents' && $_POST['contents'] != ""){
    ...
    if(isset($_POST['use_file']) && $_POST['use_file']){
        ...
        $inc_file = BASE_PATH."/".trim($_POST['site'])."/inc/".$menu_id.".php";
