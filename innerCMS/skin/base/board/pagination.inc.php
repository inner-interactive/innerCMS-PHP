<div class="pagination">
	<?php if($firstpage != -1){?>
		<a href="<?=getBackUrl("menu|no|sfv|".$_GET['sfv']."|mode|limit")?>&amp;pno=<?=$firstpage?>" class="direction prev" title="첫페이지로 이동">처음</a>
	<?php }?>
	<?php if($prepage != -1){?>
	...
	<?php }?>
	<?php if($endpage != -1){?>
		<a href="<?=getBackUrl("menu|no|sfv|".$_GET['sfv']."|mode|limit")?>&amp;pno=<?=$endpage?>" class="direction next" title="끝페이지로 이동">끝</a>
	<?php }?>
</div>